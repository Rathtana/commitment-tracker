...
export function CountCard({ goal, now, userTz, handlers, variant = 'mutable', progressDisabled = false, monthYearLabel }: CountCardProps) {
  ...
  const disabledTitle = monthYearLabel ? `Available when ${monthYearLabel} starts` : 'Not available yet'

  function commitStepper() {
    if (stepperValue === 0 || !handlers) return
